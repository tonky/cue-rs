    ...
    match ("list", func_name) {
        ...
        ("list", "Sort") => {
                ...
                && let Some(Value::List { elements, ellipsis }) = arena.get(arg0)
            {
                ...
                let el = *ellipsis;
                elems.sort_by(|&a_id, &b_id| match (arena.get(a_id), arena.get(b_id)) {
        ...
        ("list", "IsSorted") => {
                ...
                && let Some(Value::List { elements, .. }) = arena.get(arg0)
            {
                ...
                let sorted =
                    elems
                        .windows(2)
                        .all(|pair| match (arena.get(pair[0]), arena.get(pair[1])) {
        ...
        ("list", "Compact") => {
                ...
                && let Some(Value::List { elements, ellipsis }) = arena.get(arg0)
            {
                let elems = elements.clone();
                let el = *ellipsis;
                let compacted: Vec<ValueId> = elems
                    .into_iter()
                    .filter(|&e| {
                        !matches!(
                            arena.get(e),
                            Some(Value::Null) | Some(Value::Bottom(_)) | None
                        )
                    })
                    .collect();
                return Ok(arena.alloc(Value::List {
        ...
        ("list", "Chunk") => {
            ...
            {
                let elems = elements.clone();
                let chunked_lists: Vec<ValueId> = elems
                    .chunks(chunk_size)
                    .map(|chunk| {
        ...
        ("list", "Zip") => {
            if args.len() >= 2
                && let (
                    Some(Value::List { elements: l1, .. }),
                    Some(Value::List { elements: l2, .. }),
                ) = (arena.get(args[0]), arena.get(args[1]))
            {
                let l1_elems = l1.clone();
                let l2_elems = l2.clone();
                let pairs: Vec<ValueId> = l1_elems
                    .into_iter()
                    .zip(l2_elems)
                    .map(|(a, b)| {
                        arena.alloc(Value::List {
        ...
        ("list", "Unzip") => {
                ...
                && let Some(Value::List { elements, .. }) = arena.get(arg0)
            {
                ...
                let l2_id = arena.alloc(Value::List {
                    elements: l2,
                    ellipsis: None,
                });
                return Ok(arena.alloc(Value::List {
        ...
        ("list", "Slice") => {
            if args.len() >= 3
                && let (
                    ...
                    Some(Value::Int(low_val)),
                    Some(Value::Int(high_val)),
                ) = (arena.get(args[0]), arena.get(args[1]), arena.get(args[2]))
            {
                ...
                let high = high_val
                    ...
                    .min(elements.len());
                let sliced = if low <= high {
                    elements[low..high].to_vec()
                } else {
                    Vec::new()
                };
                return Ok(arena.alloc(Value::List {
