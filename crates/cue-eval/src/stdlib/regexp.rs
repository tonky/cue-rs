...
pub fn call_regexp(
    ...
    match ("regexp", func_name) {
        ...
        ("regexp", "FindAll") => {
            ...
            Err(
                "regexp.FindAll requires 1 pattern, 1 target string, and 1 limit argument"
                    .to_string(),
            )
        }
        ("regexp", "ReplaceAll") => {
            ...
            Err(
                "regexp.ReplaceAll requires 1 pattern, 1 target string, and 1 replacement"
                    .to_string(),
            )
        }
        ("regexp", "QuoteMeta") => {
            ...
            if let Some((pat, s)) = extracted {
                if let Ok(re) = Regex::new(&pat) {
                    if let Some(caps) = re.captures(&s) {
                        let match_strs: Vec<String> = caps
                            .iter()
                            .map(|m| m.map(|v| v.as_str().to_string()).unwrap_or_default())
                            .collect();
                        let matches: Vec<ValueId> =
                            match_strs.into_iter().map(|ms| arena.string(ms)).collect();
                        return Ok(arena.alloc(Value::List {
                            elements: matches,
                            ellipsis: None,
                        }));
