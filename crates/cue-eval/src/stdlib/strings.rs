...
pub fn call_strings(
    ...
    match ("strings", func_name) {
        ...
        ("strings", "ContainsAny") => {
            if args.len() >= 2
                && let (Some(Value::String(s)), Some(Value::String(chars))) =
                    (arena.get(args[0]), arena.get(args[1]))
            {
        ...
        ("strings", "ByteAt") => {
            if args.len() >= 2
                && let (Some(Value::String(s)), Some(Value::Int(idx))) =
                    (arena.get(args[0]), arena.get(args[1]))
                && let Some(i) = idx.to_usize()
            {
                ...
                if i < bytes.len() {
                    ...
                    return Err(format!(
                        "strings.ByteAt: index {i} out of range for string of length {}",
                        bytes.len()
                    ));
                }
            }
        ...
        ("strings", "ByteSlice") => {
            if args.len() >= 3
                && let (
                    Some(Value::String(s)),
                    Some(Value::Int(start_val)),
                    Some(Value::Int(end_val)),
                ) = (arena.get(args[0]), arena.get(args[1]), arena.get(args[2]))
